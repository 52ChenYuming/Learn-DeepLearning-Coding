...
class BottleNeck(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=out_channel, kernel_size=1, stride=stride,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(out_channel)
        self.conv2 = nn.Conv2d(in_channels=out_channel, out_channels=out_channel, kernel_size=3, stride=1, bias=False,
                               padding=1)
        self.bn2 = nn.BatchNorm2d(out_channel)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        downsample = None
        # channel 是每个block中第一个卷积核的个数
        # 如果满足下面则说明是50，101，152层的网络, residual块需要下采样
